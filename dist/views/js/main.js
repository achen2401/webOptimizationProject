...
var pizzaElementGenerator = function(i) {
  ...
  pizzaContainer.setAttribute("id", "pizza" + i);
  pizzaContainer.classList.add("randomPizzaContainer");
  var templateHTML ='<div class="d1"><img src="images/pizza.png" class="img-responsive"/></div><div class="d2">' + '<h4>' + randomName() + '</h4>' +'<ul>' + makeRandomPizza() +'</ul></div>';
  pizzaContainer.innerHTML = templateHTML;

  return pizzaContainer;
}

// resizePizzas(size) is called when the slider in the "Our Pizzas" section of the website moves.
// This will resize the pizza images
var resizePizzas = function(size) {
  window.performance.mark("mark_start_resize");   // User Timing API function
  var index = parseInt(size) - 1;
  //array that holds the properties for image for each size
  var sizes = [{"label": "Small", "width": "25%"}, {"label":"Medium", "width": "33.3%"}, {"label":"Large", "width": "40%"}];

  document.getElementById("pizzaSize").innerHTML = sizes[index].label;

  var pizzaContainers = document.getElementsByClassName("randomPizzaContainer");

  for (var i = 0; i < pizzaContainers.length; i++) {
    pizzaContainers[i].style.width = sizes[index].width;
  }
...
//this will calcuate the time for rendering 10 frames
function updatePositions() {
   frame++;
   window.performance.mark("mark_start_frame");

   var items = document.getElementsByClassName('mover'); //get all the moving pizzas
   var bodyScrollTop = document.body.scrollTop / 1250; //only need to do this calculation once
   var phases = []; //phases repeat every 5 times - so needs to only generate it 5 times
   for (var i = 0; i < 5; i++) {
      phases.push(Math.sin((bodyScrollTop) + (i % 5)) * 100);
   }
...
//this will dynamically generate pizza
generatePizzas();
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var colWidth = 78; //predefined calculated based on the mover element width
  var rowHeight = 100; //predefined  calculated based on the mover height
  //console.log((screen.width / colWidth) + " x " + (screen.height / rowHeight));
  var totals = Math.floor((screen.width / colWidth) * (screen.height / rowHeight));  //determine the total pizzas to render
  var anchor = document.getElementById("movingPizzas1");
